...
class Register extends SyncObserver
{
	...
	protected $pathSyncOptionQuote = 'zohocrm/sync/quotes';
...
//            $this->_lead->sync($id);
     ...
     * @return void|string
     */
    public function saveQuote($data=null)
    {
		echo 'test1';
        try {
            

            if ($this->getConfigValue($this->pathSyncOptionQuote)) {
				echo $this->getConfigValue($this->pathSyncOptionQuote);
				die('test');
                    $this->addToQueue(Queue::TYPE_QUOTE, $id);
